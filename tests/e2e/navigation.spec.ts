...
test("mvp routes are reachable", async ({ page }) => {
  ...
  await mainNav(page).getByRole("link", { name: "ビルダー", exact: true }).click();
  ...
  await mainNav(page).getByRole("link", { name: "テンプレート", exact: true }).click();
  await expect(page).toHaveURL(/\/templates$/);
  await expect(page.getByRole("heading", { level: 1, name: "テンプレート" })).toBeVisible();

  await mainNav(page).getByRole("link", { name: "設定", exact: true }).click();
